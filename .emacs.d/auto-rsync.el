...
(defcustom auto-rsync-command-option "-avzq --exclude \".git\" -e \"ssh -i /Users/mbaranch/.ssh/anpa_id_rsa\"" "rsync command option" :group 'auto-rsync)
...
(defun auto-rsync-exec-rsync ()
  ...
  (let* ((normalized-alist (mapcar (lambda (x) (cons (file-name-as-directory (expand-file-name (car x)))
                                                     (file-name-as-directory (car (cdr x)))))
